...
| `gas_highlights_v1` | マーカー `{ [qId]: [{id, choiceId, area:'choice'\|'explanation', start, end, text}] }`。`text`=マーク実文字列（再アンカー用）。**`qId` は表示中の問題ではなく「その選択肢を実際に持つ問題」のid**（後述の保存キー解決） |
...
| **マーカーの保存キー（実問題id）** | マーカーは **表示中の問題ではなく「その選択肢を実際に持つ問題」のid** をキーに保存する。`_choiceOwnerIdx`（Map: 選択肢id→実問題id・`state.questions` から遅延生成）/ `invalidateChoiceOwnerIndex()`（`saveQuestions`・`loadStoredQuestions` が呼ぶ）/ `hlKey(choiceId, fallbackQid)`（保存キー。実問題が引けなければ表示中の問題idにフォールバック）/ `hlFor(choiceId, fallbackQid)`（その選択肢のマーカー一覧）/ `hlEntriesForQuestion(q)`（表示中の問題に出る全マーカーを `[{key,h}]` で収集）/ `_hlRemoveEntries(entries)`（まとめて削除・空バケット掃除）/ `migrateHighlightsToOwnerQuestion()`（旧データの再配置。**`state.questions` ロード後**に init と `_reloadAppState` から呼ぶ）。⚠️ **`highlightsData[q.id]` と直接書かないこと**（合成問題で共有が壊れる） |
...
| **論述問題練習：入口** | ホームの「🔢 計算問題」ボタン**直下**に同サイズで並ぶ `📝 論述問題練習`（`.top-cat-essay`・`renderTopFilterCard` 内で常時表示＝データ有無に依存しない）。押すとボタン直下に科目選択ポップアップ（`.top-filter-start-popup.is-block`＝全幅で開く修飾クラス）が出る。科目は **`ESSAY_CATEGORIES`＝法令／消費機器の2つのみ**（論述が出題されるのはこの2科目だけ）・表示名は `ESSAY_CAT_LABELS`。`onEssayCategorySelect(cat)` → `openEssayList(cat)` |
| **論述問題練習：本体** | 模範解答を丸ごと登録し、一部を塗りつぶして暗記するモード。画面は3つ＝`screen-essay-list`（科目ごとの一覧）/ `screen-essay-practice`（練習）/ `screen-essay-edit`（登録・編集）。データは `essayNotes`（`gas_essay_notes_v1`・**`BACKUP_LS_KEYS` 登録済み**）＝`[{id,category,title,body,blanks:[{id,start,end,text}],created,updated}]`。**`blanks` の `start/end` は `renderText(body)` の可視テキスト上のオフセットで、`text`（実文字列）を併せ持つ＝マーカーと同じ再アンカー方式**なので本文を編集しても位置が追従する（`_anchorHighlight` を共用）。描画も `_applyHighlightRange` を共用（クラスは練習=`essay-blank`／編集=`essay-blank-edit`）|
| **論述：レベルと隠し方** | `ESSAY_LEVEL_PCT = [0,20,40,60,80,100]`（index=レベル0〜5）。`rerollEssayHidden()` が **blanks の「個数」に対する割合ぶんを毎回ランダム抽選**して `essayHiddenIds` に入れる（文字数基準ではない／レベル0=全文表示・レベル5=全部隠す／最低1件）。同じレベルでも「🔀 隠し直す」で別の箇所が隠れる。隠れた部分は**クリックで表示/非表示をトグル**（`.revealed`）、「👁 全部表示 / 🙈 全部隠す」で一括。`_updateEssayRevealBtn()` がラベルを状態に追従させる |
| **論述：一覧の並べ替えとJSON入出力** | 並べ替えは `essaySortMode`（`title-asc`/`title-desc`/`registered-asc`/`registered-desc`・既定=`title-asc`）／`sortEssayNotes()`／`_updateEssaySortBtns()`（`.essay-sort-btn` の active と ↑↓ を更新）。⚠️ **タイトル比較は必ず `normalize('NFKC')` してから** `localeCompare(…,'ja',{numeric:true})` する。丸数字をそのまま比べると **⑩ が ① より前**に来る（NFKCで ①→"1"・⑩→"10" に開けば数値順になり、全角英数・半角カナの表記ゆれも吸収できる）。JSONは `exportEssayNotes()`（`{type:'gas-essay-notes',version:1,notes:[…]}` を `essay_notes_YYYY-MM-DD.json` で保存）/ `importEssayNotes(file)`（**id が一致すれば上書き・無ければ追加＝全消しはしない**。`notes` 配列そのものの形式も受け付ける。title/body が無い要素は読み飛ばして件数を報告、未知の `category` は既定科目へ寄せる）|
| **論述：範囲指定の操作** | 編集画面のプレビュー（`#essay-edit-preview`）で **ドラッグ＝隠す範囲を作成（重なり・隣接は融合）／クリック＝解除**。⚠️ **作成と解除は両方 `onEssayPreviewMouseUp` の中で処理する**。mouseup=作成・click=解除に分けると、ドラッグ直後に発火する click が作ったばかりの範囲を消す。それをフラグで抑えると、click が来ない操作（範囲外でドラッグを終える等）でフラグが残り次の正当なクリックが無視される。**プレビューは `#screen-drill`／`#choices-list` の外なので、既存のマーカー用 document mouseup ハンドラとは競合しない**。編集中に本文と一致しなくなった範囲は**プレビューでは消さず**（入力途中の一時的な不一致があるため）、保存時に確認ダイアログを出してから落とす |
...
| **解説画像（問題全体）** | `q.explanationImage`。**学習・壁打ちの両方**でトグルボタン（「🖼 解説画像を表示／閉じる」）付きで表示する。描画は共通関数 `renderQuestionExplanationImage(areaEl, q)`（`q` が falsy／画像なしなら領域を空にして `hidden`）。差し込み先は学習=`#explanation-image-area`（`checkAnswers` の2経路）、壁打ち=`#drill-explanation-image-area`（`answerDrill` と `renderDrillChoice` の回答済み復元パス。未回答パスでは `null` を渡して前の選択肢の画像を消す）。**選択肢ごとの `c.explanationImage`（常時表示）とは別物** |
...
| **今日の学習のティア（銅〜ダイヤ）** | しきい値は **`TODAY_TIER_STEPS` が唯一の定義**（選択肢数で 40=銅／80=銀／120=金／160=プラチナ／200=ダイヤ）。`tierForChoices(n)`（ティア名）と `nextTierRemaining(n)`（次のティアまでの残り）を経由して、①ヘッダーの `hd-tier-*` ②「次のステータスまで後N問」③学習カード枠線の `getTodayTier()`→`tier-border-*` ④**カレンダーの日別メダル**（`renderCalendar` の `tier-*` クラス＋`CAL_CELL` 対応表でメダルSVGを選ぶ。`copper` だけ生成関数名が `_makeBronzeCalCell` なので対応表で吸収）の4箇所が導出される。⚠️ **以前は同じ数値が3箇所にベタ書きで変更漏れの温床だった。しきい値を変えるときは `TODAY_TIER_STEPS` だけを編集すること。** なお `getTodayTier()` は studyLog のみを見るため、**外部学習を入力した日はヘッダー（外部学習を加算した `todayChoices` で判定）とカード枠線がずれる**（仕様）。結果画面の `examScoreTier(pct)` は正答率ベースの別物 |
...
- **`<button>` で作った要素には `color` と `font-family` を必ず明示する**。button は親の `color`／`font-family` を**継承しない**（UA既定の黒文字＋別フォントになる）ため、暗い背景のカード上に置くと**黒文字で読めなくなる**。既存の `.btn` は `color: var(--text-1)` を持っているので `.btn` を付ければ安全だが、独自クラスだけのボタン（`.essay-list-item`・`.essay-level-btn` など）は自分で指定すること。`div` で作った一覧項目（`.calc-practice-item`）は継承するので不要 —— **同じ見た目のつもりで button と div を混ぜると片方だけ黒くなる**。副次情報の色に `--text-3` を使うと暗背景でコントラスト比が3.0未満になり読みづらいので、一覧の補足テキストは `--text-2` を使う。
- **画像は4種類あり、どれも学習・壁打ちの両方に出す**：①本文ブロック画像（`questionBlocks` の `type:'image'`／`renderBlocksToEl`）②選択肢画像（`c.image`）③選択肢ごとの解説画像（`c.explanationImage`／常時表示）④**問題全体の解説画像（`q.explanationImage`／トグル式）**。④は長らく学習画面にしか実装されておらず、「通常問題の編集で貼った画像が壁打ちに反映されない」不具合になっていた（①②③は両画面にあった）。現在は `renderQuestionExplanationImage()` を共用。**編集モーダルに新しい画像の貼り付け先を足したら、必ず4種類すべての表示経路（学習の `checkAnswers` 2箇所・`renderDrillChoice`・`answerDrill`・`openViewModal`・振り返りモーダル）を確認すること。**
- **マーカーは「実問題id」に保存する（合成問題との共有）**：キーワード出題の📄通常出題は `kwset-<timestamp>-<n>`、出題ジェネレータの選択肢単位は `gen-<cat>-q<N>` という **実問題とは別idの合成問題** を作る。ここで `highlightsData[表示中のq.id]` に保存すると、同じ選択肢なのに壁打ち（実問題id）とバケットが分かれ、**通常出題で引いた赤太文字が壁打ちに出ない**（`kwset-*` は毎回idが変わるので次回以降どこからも見えなくなる）。対策として保存・読み出し・削除のすべてを `hlKey(choiceId, fallbackQid)`＝**その選択肢を持つ実問題のid** 経由にした。`_applyHighlights` は `highlightsData[q.id]` を舐めるのではなく `hlEntriesForQuestion(q)`（＝q.choices ごとに実問題バケットから収集）を使う。⚠️ **マーカーを読み書きするコードを足すときは必ず `hlKey`／`hlFor`／`hlEntriesForQuestion`／`_hlRemoveEntries` を通すこと。** 進捗・ブックマークが選択肢idに紐づいて合成問題でも連動するのと同じ理屈。旧データは `migrateHighlightsToOwnerQuestion()` が起動時に実問題idへ移す（実問題が見つからないエントリは**消さずに残す**）。
