...
| **論述問題練習（実装途中）** | ホームの「🔢 計算問題」ボタン**直下**に同サイズで並ぶ `📝 論述問題練習`（`.top-cat-essay`・`renderTopFilterCard` 内で常時表示＝データ有無に依存しない）。押すとボタン直下に科目選択ポップアップ（`.top-filter-start-popup.is-block`＝全幅で開く修飾クラス）が出る。科目は **`ESSAY_CATEGORIES`＝法令／消費機器の2つのみ**（論述が出題されるのはこの2科目だけ）・表示名は `ESSAY_CAT_LABELS`。状態は `essayPickerOpen`（ポップアップ開閉）/ `essayCategory`（選択中の科目・同じ科目を再選択で解除）。⚠️ **`onEssayCategorySelect` は選択を保持して案内トーストを出すだけで、出題・採点画面は未実装**。次のステップはここから遷移させる |
